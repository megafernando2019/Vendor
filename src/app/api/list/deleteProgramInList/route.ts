...
import {
  deleteProgramInList,
  type DeleteProgramInListPayload,
} from "@/services/list";
...
export async function POST(req: Request) {
  ...
  try {
    const body = (await req.json()) as Partial<DeleteProgramInListPayload>;
    ...
    if (!mt) {
      return NextResponse.json(
        { success: false, message: "mt es requerido" },
        { status: 422 },
      );
    }

    const payload: DeleteProgramInListPayload = {
      user_id: userId,
      list_name: listName,
      mt,
    };

    const res = await deleteProgramInList(token, payload);
    ...
    return NextResponse.json(
      { success: false, message: "Error al eliminar el programa de la lista" },
      { status: 500 },
    );
  }
}
