...
import faq_data, { flattenFaqData } from "@/data/FaqData";
...
const FaqArea = () => {
  const [faqData, setFaqData] = useState<FaqAccordionItem[]>([]);

  useEffect(() => {
    const items = flattenFaqData(faq_data);
    const updatedData = items.map((item) => ({
      ...item,
      showAnswer: item.id === items[1]?.id,
    }));
              ...
              <div className="tg-faq-content">
                <div className="accordion tg-custom-accordion" id="accordionExample">
                  {faqData.map((item) => (
                    <div
                      key={item.id}
                      className={`accordion-item ${item.showAnswer ? "tg-faq-active" : ""} mb-10 wow fadeInUp`}
                      data-wow-delay=".3s"
                      data-wow-duration=".9s"
                    >
                      <h2
